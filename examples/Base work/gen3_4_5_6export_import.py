...
def run_simulation(model):
    ...
    while t < 10:
        ...
        # ----------------------------------------
        # APPLY STEP ONCE (ALL LOCAL GENERATORS)
        # ----------------------------------------
...
for label, Pval in power_cases.items():
    ...
    model_case = apply_full_system(base_model)
    model_case = set_power(model_case, Pval)

    print(f"\nRunning CASE 4: {label}")
...
plt.title("Active Power of G3 (Case 4 - Strong Grid)")
plt.xlabel("Time [s]")
plt.ylabel("MW")
plt.legend()
plt.grid()
...
plt.title("Voltage at B6 (Case 4 - Strong Grid)")
plt.xlabel("Time [s]")
plt.ylabel("pu")
plt.legend()
plt.grid()
...
plt.title("Reactive Power of G3 (Case 4 - Strong Grid)")
plt.xlabel("Time [s]")
plt.ylabel("MVAr")
plt.legend()
plt.grid()
