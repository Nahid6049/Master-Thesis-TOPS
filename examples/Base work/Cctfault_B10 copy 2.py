...
def run_sim(active_gens, fault_duration, record=False):
    ...
    while sol.t < SIM_END:
        ...
        # fault ON during [FAULT_START, FAULT_START + fault_duration)
        ...
        # instability rule:
        # if ANY active local generator slips beyond pi rad relative to G1
...
def plot_case_result(label, case_result, active_gens):
    ...
    plt.title(f"CCT Analysis at Bus B10 ({label}): Average Local Rotor Angle Relative to G1")
    plt.xlabel("Time [s]")
    plt.ylabel("Rotor angle [rad]")
    plt.grid()
    plt.legend()

    # 2) each local generator angle
    plt.figure()
    if stable_res is not None:
        for g in local_names:
    ...
    plt.axhline(np.pi, linestyle=':', color='k', label='±π limit')
    plt.axhline(-np.pi, linestyle=':', color='k')
    plt.title(f"CCT Analysis at Bus B10 ({label}): Individual Local Generator Angles Relative to G1")
    plt.xlabel("Time [s]")
    plt.ylabel("Rotor angle [rad]")
    plt.grid()
    plt.legend()

    # 3) voltage at fault bus
    plt.figure()
    if stable_res is not None:
    ...
    plt.title(f"CCT Analysis at Bus B10 ({label}): Voltage Response at Bus B10")
    plt.xlabel("Time [s]")
    ...
    plt.title(f"CCT Analysis at Bus B10 ({label}): VSC Reactive Power Response")
    plt.xlabel("Time [s]")
    plt.ylabel("Reactive power [MVAr]")
    plt.grid()
    plt.legend()

    # 5) average local speed
    plt.figure()
    if stable_res is not None:
    ...
    plt.title(f"CCT Analysis at Bus B10 ({label}): Average Local Generator Speed")
    plt.xlabel("Time [s]")
    plt.ylabel("Speed [pu]")
    plt.grid()
    plt.legend()
